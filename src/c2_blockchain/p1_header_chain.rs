...
impl Header {
    ...
    fn verify_sub_chain(&self, chain: &[Header]) -> bool {
        ...
        for i in 0..chain.len() {
            let parent_hash = chain[i].parent;
            let (parent_height, overflow) = chain[i].height.overflowing_sub(1);
            if i == 0 {
                is_verified =
                    !overflow && parent_hash == hash(self) && parent_height == self.height;
            } else {
                let parent = &chain[i - 1];
                is_verified =
                    !overflow && parent_hash == hash(parent) && parent_height == parent.height;
            }
